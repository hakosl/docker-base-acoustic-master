...
class Sigmoid_Log_Separate_Freq(nn.Module):
    """
    Preprocessing of input pixels.
    Equivalent to 'x = sigmoid(log(x))', with trainable Sigmoid parameters 'a' and 'k'.
    Separate parameters for each input frequency.
    """

    def __init__(self):
        super(Sigmoid_Log_Separate_Freq, self).__init__()
        self.eps = 1e-25
        self.k = nn.Parameter(torch.Tensor(preprocess_params()['k']).reshape(1, 4, 1, 1))
        self.a = nn.Parameter(torch.Tensor(preprocess_params()['a']).reshape(1, 4, 1, 1))
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # preprocessing input with sigmoid-log function
        x = self.preprocess(x)
