...
def partition_data(echograms, partition='random', portion_train=0.85):
    ...
    if partition == 'random':
        ...
        #     ['D2008', 'D2009', 'D2010', 'D2011', 'D2012', 'D2013', 'D2014', 'D2015', 'D2016']]), echograms))
        train = list(filter(lambda x: any(
            [year in x.name for year in
             ['D2011', 'D2012', 'D2013', 'D2014', 'D2015','D2016']]), echograms))
        test = list(filter(lambda x: any([year in x.name for year in ['D2017']]), echograms))
        #val = list(filter(lambda x: any([year in x.name for year in ['D2007']]), echograms))
        val = []
...
def rand_init_fn(i):
    return np.random.seed(i + np.random.randint(0, 2**20))
    #return np.random.seed(i) #Use this line instead for training on the same sequence of data samples each run.
...
def train_model(
    ...
    '''
    Train segmentation model with 3 classes - sandeel/other/background
    ...
    :param path_model_params_load: (str) File path to pretrained model parameters to initialize model
    :param path_model_params_save: (str) File path to save model parameters after training
    ...
    if load_pre_trained:
        model_state_dict = torch.load(path_model_params_load)
    ...
    data_transform = CombineFunctions([remove_nan_inf, db_])
    ...
    dataloader_train = DataLoader(
        dataset_train, batch_size=batch_size, shuffle=False, num_workers=num_workers, worker_init_fn=rand_init_fn)
    ...
    for i, (inputs_train, labels_train) in enumerate(dataloader_train):
        ...
        # Todo: Flytte optimizer.step() opp (etter loss_train.backward())? Har dette noe å si?
        # Har flyttet fra over running_loss_train += loss_train.item()
        # Optimize
        optimizer.step()
        ...
        # Print loss, accuracy and confusion matrix
        if (i + 1) % log_step == 0:
            model.eval()
            with torch.no_grad():
                labels_true = []
...
if __name__ == '__main__':
    ...
    train_model(
        dev='', # Insert name of cuda device here - e.g. 'cuda:0'
        ...
        lr_step=1000,
        iterations=5000,
        path_model_params_load='', # Insert path here if loading pretrained model params
        path_model_params_save='', # Insert path here if saving model params after training
        partition='year',
        load_pre_trained=False,
        save_model_params=False
    )