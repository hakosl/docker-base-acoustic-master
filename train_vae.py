...
from batch.file_dataset import acoustics_dataset
...
def train_model(
    ...
    dataset_train = acoustics_dataset("data/train.npy")
    dataset_val = acoustics_dataset("data/validation.npy")
    dataset_test = acoustics_dataset("data/test.npy")
    dataloader_train = DataLoader(dataset_train, batch_size=81, num_workers=num_workers)
    dataloader_test = DataLoader(dataset_test, batch_size=81, num_workers=num_workers)
    dataloader_val = DataLoader(dataset_val, batch_size=81, num_workers=num_workers)
    ...
    for epoch in range(epochs):
        ...
        delta = print_loss_save_latent(model, inputs_test, start_time, i, iterations, base_figure_dir, recon_criterion, variational_beta, running_loss_train, running_recon_loss, running_kl_loss, log_step, losses, recon_losses, kl_losses, iteration, device, si_test, latent_image_fns, verbose, writer, epoch, explicitness)
    ...
    best_r_score, cm, clf, clf_acc = validate_clustering(model, HDBSCAN(prediction_data=True), dataloader_val, dataloader_test, dataset_test.get_label_names(), device, model.capacity, variational_beta, fig_path=fig_path, i=i, writer=writer)
